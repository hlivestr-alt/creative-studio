...
5. Open the desktop app, select H3 → Auto Run, choose products/content types, and press Start when ready. Start checks the archive route and both output roots before creating the session.

For a different archive location, set `PROYA_H3_ARCHIVE_ROOT` in the local ComfyUI process environment before startup and enter the same absolute path under Auto Run → Output in the desktop app. An application request cannot change the server's allowed archive root.
...
- `GET /proya/auto/archive/file`: serves only MP4 files inside the configured archive root for resumable local download attempts.
...
The full globally unique job ID remains in the ComfyUI SaveVideo prefix, workflow metadata, and Creative Studio SQLite records. The archive/output filename contains its 12-character unique suffix to keep Windows paths short. The job record maps both final paths to the full identity.

Archive copy failure is recorded separately and retried; it never requests another render. Local output copies read the archive rather than depending on the original output. Downloaded files must be nonempty and match the archived byte count before promotion from a temporary file. One local output copy runs at a time.
