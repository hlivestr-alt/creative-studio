...
describe('Local authoritative session compatibility mirror', () => {
  it('persists visibility metadata only, with no scheduler fields', async () => {
    ...
    try {
      ...
      db.saveLocalAutoMirror({ sessionId: 'session', lastKnownRevision: 5, bundleHash: 'a'.repeat(64), runnerVersion: 'r7', connectionState: 'connected', lastSuccessfulSync: '2026-09-09T00:00:00.000Z', settingsVersionIdentity: 'b'.repeat(64), stagingState: 'STAGED_READY', createdTimestamp: '2026-09-09T00:00:00.000Z' });
      const mirror = db.getLocalAutoMirror('session');
