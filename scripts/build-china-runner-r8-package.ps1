...
$source = Join-Path $repoRoot 'deploy\china-auto-runner-canary'
$shared = Join-Path $repoRoot 'deploy\china-auto-runner-shadow'
$output = Join-Path $repoRoot 'release\china-auto-runner-canary-20260909-r8'
$runtime = Join-Path $output 'runtime'
$zip = Join-Path $repoRoot 'release\PROYA-China-Auto-Runner-Canary-20260909-r8.zip'
$deploymentManifest = Join-Path $repoRoot 'release\PROYA-China-Auto-Runner-Canary-20260909-r8.manifest.json'
function Hash([string]$path) { (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() }
if ((Test-Path $zip) -or (Test-Path $deploymentManifest)) { throw 'Refusing to overwrite an existing r8 artifact.' }
...
$manifest=[ordered]@{ filename=(Split-Path -Leaf $zip); runnerVersion='1.2.0-canary.20260909'; packageRelease='20260909-r8'; mode='canary'; maxJobsPerSession=1; buildTimestamp=$timestamp; listenAddress='127.0.0.1:8787'; comfyUrl='http://127.0.0.1:8188'; lmStudioUrl='http://127.0.0.1:1234'; cloudflareRequiredAfterStart=$false; canaryStartEnabled=$true; continuousAutoRunEnabled=$false }
