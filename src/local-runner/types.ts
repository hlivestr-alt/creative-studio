...
export const LOCAL_RUNNER_VERSION = '2.1.4-production.20260912';
...
export interface LocalSessionBundle {
  sessionId: string;
  ...
  /** Additive text-only graph, present when Support B-Roll is selected. */
  supportBRollWorkflow?: Record<string, unknown>;
  supportBRollWorkflowSha256?: string;
  supportBRollWorkflowVersion?: string;
  archiveRoot: string;
...
export interface PersistedJob {
  /** Chosen once at job creation, including for CTA; recovery reads this value. */
  durationSeconds?: number;
  /** Chosen once for a new Hook job and persisted for recovery. */
  hookArchetype?: import('../domain/hook-archetype').HookArchetype;
  ctaStyle?: import('../domain/cta-settings').CtaStyle;
  ctaPreviousStyle?: import('../domain/cta-settings').CtaStyle;
  jobId: string;
  sessionId: string;
  schedulerKey: string;
  phase: JobPhase;
