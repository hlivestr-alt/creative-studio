...
import type { LocalGenerationRunner } from './runner';
import type { LocalSessionBundle } from './types';
...
export function createRunnerApi(runner: LocalGenerationRunner): Server {
  return createServer(async (request, response) => {
    try {
      ...
      if (method === 'POST' && url.pathname === '/proya/auto/stage') return json(response, 201, await runner.stage(await body(request) as LocalSessionBundle));
      ...
      if (syncMatch) {
        ...
        return json(response, 200, { job: runner.acknowledgeLocalOutputSync(decodeURIComponent(syncMatch[1]), input.path) });
      }
