...
const roots: string[] = []; const runners: LocalGenerationRunner[] = [];
...
function bundle(root: string, product = products[0]): LocalSessionBundle {
...
function runnerWith(executor: CanaryExecutor, root = mkdtempSync(join(tmpdir(), 'proya-canary-')), interval = 60_000) {
  roots.push(root); const clients = localClients(); const runner = new LocalGenerationRunner({ mode: 'canary', stateRoot: join(root, 'state'), archiveRoot: join(root, 'archive'), canaryExecutor: executor, schedulerIntervalMs: interval, ...clients }); runners.push(runner); return { runner, root };
}
...
describe('China-owned one-job canary scheduler', () => {
  ...
  it('continues independently after Start and recovers RUNNING state after runner restart', async () => {
    ...
    const clients = localClients(); const recovered = new LocalGenerationRunner({ mode: 'canary', stateRoot: join(root, 'state'), archiveRoot: join(root, 'archive'), canaryExecutor: completeExecutor(), schedulerIntervalMs: 60_000, ...clients }); runners.push(recovered);
