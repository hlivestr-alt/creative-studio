...
import { LOCAL_RUNNER_VERSION, type LocalSessionBundle } from './types';
...
function bundle(root: string): LocalSessionBundle {
  ...
  const value: LocalSessionBundle = {
    sessionId: randomUUID(), schemaVersion: 1, expectedRunnerVersion: LOCAL_RUNNER_VERSION,
...
function runner(executor: CanaryExecutor, root = mkdtempSync(join(tmpdir(), 'proya-production-')), interval = 60_000) {
  roots.push(root);
  const value = new LocalGenerationRunner({ mode: 'production', stateRoot: join(root, 'state'), archiveRoot: join(root, 'archive'), canaryExecutor: executor, schedulerIntervalMs: interval, ...clients() });
  runners.push(value);
...
describe('final local production scheduler', () => {
  it('resumes a STOPPING current job after restart through archive, /free, and fresh VRAM verification without scheduling another job', async () => {
    const promptId = randomUUID();
    const first = runner({ executeStep: async (_session, job, hooks) => hooks.update('RUNNING', { promptId: job.promptId ?? promptId }) });
    const staged = bundle(first.root);
    await first.runner.stage(staged); await first.runner.startCanary(staged.sessionId, staged.bundleSha256);
    await waitFor(() => first.runner.jobs(staged.sessionId)[0]?.phase === 'RUNNING');
    const cursor = first.runner.session(staged.sessionId)!;
    first.runner.requestStopAfterCurrent(staged.sessionId);
    expect(first.runner.session(staged.sessionId)).toMatchObject({ status: 'STOPPING', stopAfterCurrent: true, currentJobId: first.runner.jobs(staged.sessionId)[0].jobId });
    first.runner.close(); runners.splice(runners.indexOf(first.runner), 1);

    const phases: string[] = [];
    const recovered = new LocalGenerationRunner({ mode: 'production', stateRoot: join(first.root, 'state'), archiveRoot: join(first.root, 'archive'), schedulerIntervalMs: 10, ...clients(), canaryExecutor: { executeStep: async (session, job, hooks) => {
      expect(session.status).toBe('STOPPING');
      expect(job.promptId).toBe(promptId);
      phases.push(job.phase);
      if (job.phase === 'RUNNING') return hooks.update('OUTPUT_CAPTURED', { outputFilename: 'current.mp4', outputCapturedAt: new Date().toISOString() });
      if (job.phase === 'OUTPUT_CAPTURED') return hooks.update('ARCHIVED', { archivePath: 'D:/AI Videos/current.mp4', archiveSize: 10, archiveSha256: 'c'.repeat(64), archivedAt: new Date().toISOString() });
      if (job.phase === 'ARCHIVED') return hooks.update('RELEASING_VRAM', { releaseAttempts: 1 });
      if (job.phase === 'RELEASING_VRAM') return hooks.update('COMPLETED', { vramAudit: verified() });
      return job;
    } } });
    runners.push(recovered);
    await waitFor(() => recovered.session(staged.sessionId)?.status === 'STOPPED');
    expect(phases).toEqual(['RUNNING', 'OUTPUT_CAPTURED', 'ARCHIVED', 'RELEASING_VRAM']);
    expect(recovered.jobs(staged.sessionId)).toHaveLength(1);
    expect(recovered.jobs(staged.sessionId)[0]).toMatchObject({ phase: 'COMPLETED', promptId, releaseAttempts: 1, vramAudit: verified() });
    expect(recovered.session(staged.sessionId)).toMatchObject({ status: 'STOPPED', currentJobId: null });
    expect([recovered.session(staged.sessionId)?.cycleNumber, recovered.session(staged.sessionId)?.productIndex, recovered.session(staged.sessionId)?.contentTypeIndex]).toEqual([cursor.cycleNumber, cursor.productIndex, cursor.contentTypeIndex]);
  });

  it('reconciles terminal Comfy history on the first STOPPING tick without resubmitting or creating a next job', async () => {
    const promptId = randomUUID(); let calls = 0;
    const created = runner({ executeStep: async (session, job, hooks) => {
      calls++;
      if (job.phase === 'PREPARING') return hooks.update('RUNNING', { promptId });
      expect(session.status).toBe('STOPPING');
      expect(job.promptId).toBe(promptId);
      return hooks.update('COMPLETED', { archivePath: 'D:/AI Videos/current.mp4', archiveSize: 10, archiveSha256: 'd'.repeat(64), releaseAttempts: 1, vramAudit: verified() });
    } }, undefined, 10);
    const staged = bundle(created.root);
    await created.runner.stage(staged); await created.runner.startCanary(staged.sessionId, staged.bundleSha256);
    await waitFor(() => created.runner.jobs(staged.sessionId)[0]?.phase === 'RUNNING');
    created.runner.requestStopAfterCurrent(staged.sessionId);
    ...
    expect(created.runner.jobs(staged.sessionId)[0]).toMatchObject({ phase: 'COMPLETED', promptId });
  });

  it('advertises unlimited production and loops product/content order across cycles with one GPU job at a time', async () => {
    let active = 0; let maximum = 0; const owner: { runner?: LocalGenerationRunner } = {};
  ...
  it('keeps current settings immutable, applies the next committed version, and advances after validation failure', async () => {
    const owner: { runner?: LocalGenerationRunner } = {};
  ...
  it('blocks the next position on unsafe VRAM evidence and resumes the same logical position after restart', async () => {
    ...
    first.runner.close(); runners.splice(runners.indexOf(first.runner), 1);
    const recovered = new LocalGenerationRunner({ mode: 'production', stateRoot: join(first.root, 'state'), archiveRoot: join(first.root, 'archive'), schedulerIntervalMs: 10, ...clients(), canaryExecutor: { executeStep: async (session, job, hooks) => { if (job.phase === 'FAILED') return job; return hooks.update('FAILED', { error: 'unexpected' }); } } });
