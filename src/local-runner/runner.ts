...
import { createH3ReferencePlan, createOptionalH3ReferencePlan, h3WorkflowSettingsFromBrief } from '../domain/h3';
...
import { isCtaEndCard } from '../domain/cta-end-card';
import { selectHookArchetype } from '../domain/hook-archetype';
import type { H3VideoBrief } from '../domain/types';
import { LocalComfyClient, LocalLmStudioClient } from './localhost-comfy';
import { LocalRunnerStore } from './store';
import { stageSessionBundle } from './staging';
import { LocalCanaryExecutor, type CanaryExecutor } from './canary-executor';
import { LOCAL_RUNNER_VERSION, DEFAULT_API_ADDRESS, DEFAULT_API_PORT, DEFAULT_ARCHIVE_ROOT, DEFAULT_STATE_ROOT, LOCAL_COMFY_URL, LOCAL_LM_STUDIO_URL, SESSION_BUNDLE_SCHEMA_VERSION, type CanaryStartResult, type LocalSessionBundle, type PersistedJob, type PersistedSession, type RunnerCapabilities, type RunnerMode, type SessionSettings, type StageResult } from './types';
...
export class LocalGenerationRunner {
  readonly stateRoot: string;
  readonly archiveRoot: string;
  readonly address: string;
  readonly port: number;
  readonly store: LocalRunnerStore;
  ...
  constructor(options: LocalRunnerOptions = {}) {
    ...
    this.store = options.store ?? new LocalRunnerStore(join(this.stateRoot, 'runner.sqlite3'));
  ...
  async stage(bundle: LocalSessionBundle): Promise<StageResult> {
  ...
  acknowledgeLocalOutputSync(jobId: string, path: string): PersistedJob {
  ...
  private planJob(sessionId: string, status: PersistedSession['status'], eventType: string): PersistedJob {
    ...
    // A preselected value is used only by deterministic internal QA fixtures.
    // Ordinary app-created bundles never set it, so every genuinely new Hook
    // job takes the weighted draw here and persists the result below.
    brief.hookArchetype = contentType === 'Hook' ? brief.hookArchetype ?? selectHookArchetype() : undefined;
    ...
    const primary = binding?.assetIds[0] ? session.bundle.assets.find(asset => asset.id === binding.assetIds[0]) : undefined;
    if (!supportBRoll && !primary) throw new Error(`Staged master ${binding?.assetIds[0]} is missing.`);
    if (!supportBRoll && primary) brief.references.productReference = { source: 'selected-product', description: `${product.officialName} packaging reference`, path: join(session.sessionDirectory, 'assets', `${primary.id}__${primary.filename}`) };
    ...
    const job = this.store.createJob({
      ...
      product: product.id, contentType, durationSeconds: brief.duration, settingsVersion: session.settingsVersion,
      hookArchetype: brief.hookArchetype,
      settingsSnapshot: { ...structuredClone(settings), brief: structuredClone(brief), h3: validateH3WorkflowSettings(h3WorkflowSettingsFromBrief(brief)) }, creativeSeed,
      ctaPreviousStyle: isCtaEndCard(contentType) ? this.store.listJobs(sessionId).filter(item => item.product === product.id && isCtaEndCard(item.contentType) && item.ctaStyle).at(-1)?.ctaStyle : undefined,
      creativeGenome: plan.genome, creativeFingerprint: plan.fingerprint.signature, generationBrief,
      referenceAssetIds: supportBRoll ? [] : [...binding!.assetIds], promptId: null, submissionHash: null,
  ...
  private async resumeCanaries(): Promise<void> {
    ...
    for (const session of this.store.listSessions().filter(candidate =>
      ['CANARY_STARTING', 'CANARY_RUNNING', 'TWO_JOB_CANARY_STARTING', 'TWO_JOB_CANARY_RUNNING', 'PRODUCTION_STARTING', 'PRODUCTION_RUNNING'].includes(candidate.status)
      || candidate.status === 'STOPPING' && candidate.stopAfterCurrent && candidate.currentJobId !== null
  ...
  private async driveCanary(sessionId: string): Promise<void> {
    ...
    try {
      ...
      while (!this.closed) {
        ...
        // CTA jobs can finish far faster than the observer's 2 s refresh. Give the
        // operator a bounded opportunity to commit Stop After Current before the
        // next CTA is planned; existing H3 scheduling remains untouched.
        if (isCtaEndCard(result.contentType)) await new Promise(resolveDelay => setTimeout(resolveDelay, 5_000));
