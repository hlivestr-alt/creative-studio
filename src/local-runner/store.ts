...
import type { LocalSessionBundle, JobPhase, PersistedJob, PersistedSession, SessionSettings } from './types';

const { DatabaseSync } = createRequire(`${process.cwd()}/proya-local-runner-native.cjs`)('node:sqlite') as typeof import('node:sqlite');
...
export class LocalRunnerStore {
  ...
  currentSession(): PersistedSession | null {
    // "Current" is the newest authoritative lifecycle record, including a
    // terminal one. Filtering terminal sessions lets an abandoned older
    // STAGED record become current again after a newer run stops.
    const row = this.database.prepare('SELECT json FROM sessions ORDER BY revision DESC LIMIT 1').get() as SessionRow | undefined;
    return row ? JSON.parse(row.json) as PersistedSession : null;
  }
...
export function newPersistedSession(bundle: LocalSessionBundle, bundleHash: string, sessionDirectory: string): PersistedSession {
  const timestamp = now();
  return {
