...
async function main(): Promise<void> {
  ...
  const canary = new LocalGenerationRunner({ mode: 'canary', stateRoot: join(root, 'state'), archiveRoot: join(root, 'archive'), schedulerIntervalMs: 60_000,
  ...
  const twoJob = new LocalGenerationRunner({ mode: 'two-job-canary', stateRoot: join(twoJobRoot, 'state'), archiveRoot: join(twoJobRoot, 'archive'), schedulerIntervalMs: 60_000,
    comfy: { readiness: async () => probe } as unknown as LocalComfyClient,
    lmStudio: { readiness: async () => probe } as unknown as LocalLmStudioClient,
    canaryExecutor: { executeStep: async (_session, job) => job }
  });
  ...
  const production = new LocalGenerationRunner({ mode: 'production', stateRoot: join(productionRoot, 'state'), archiveRoot: join(productionRoot, 'archive'), schedulerIntervalMs: 60_000,
    comfy: { readiness: async () => probe } as unknown as LocalComfyClient,
    lmStudio: { readiness: async () => probe } as unknown as LocalLmStudioClient,
    canaryExecutor: { executeStep: async (_session, job) => job }
  });
