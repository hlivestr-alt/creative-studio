...
function testBundle(root: string): LocalSessionBundle {
  ...
  const bundle: LocalSessionBundle = {
    ...
    schemaVersion: 1,
    expectedRunnerVersion: LOCAL_RUNNER_VERSION,
...
function createRunner(executor: CanaryExecutor, root = mkdtempSync(join(tmpdir(), 'proya-two-job-')), interval = 60_000) {
  roots.push(root);
  const runner = new LocalGenerationRunner({ mode: 'two-job-canary', stateRoot: join(root, 'state'), archiveRoot: join(root, 'archive'), canaryExecutor: executor, schedulerIntervalMs: interval, ...localClients() });
  runners.push(runner);
  return { runner, root };
}
...
describe('Local two-job canary scheduler', () => {
  ...
  it('Stop After Current is persisted and prevents Job 2', async () => {
    const runnerRef: { current?: LocalGenerationRunner } = {};
    const executor: CanaryExecutor = {
  ...
  it('recovers after Job 1 VRAM verification without duplicating either job', async () => {
    ...
    const recovered = new LocalGenerationRunner({ mode: 'two-job-canary', stateRoot: join(created.root, 'state'), archiveRoot: join(created.root, 'archive'), canaryExecutor: recoveredExecutor, schedulerIntervalMs: 10, ...localClients() });
  ...
  it('continues the Job 1 to Job 2 handoff while auxiliary visibility is disconnected', async () => {
  ...
  it.each([
    ...
    const recovered = new LocalGenerationRunner({ mode: 'two-job-canary', stateRoot: join(created.root, 'state'), archiveRoot: join(created.root, 'archive'), schedulerIntervalMs: 10, ...localClients(), canaryExecutor: {
  ...
  it('contains no external generation dependency and pins both internal services to loopback', () => {
    const source = ['runner.ts', 'canary-executor.ts', 'localhost-comfy.ts', 'types.ts']
      .map(name => readFileSync(join(process.cwd(), 'src', 'local-runner', name), 'utf8')).join('\n');
