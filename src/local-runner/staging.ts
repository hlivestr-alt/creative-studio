...
import { validateMiniMaxH3ApiWorkflowTemplate, validateMiniMaxH3T2VAApiWorkflowTemplate } from '../domain/minimax-h3-workflow';
import { h3ContentTypeOptions } from '../domain/h3';
import { legacyH3ContentTypes } from '../domain/types';
import { isNoProductVideo } from '../domain/support-b-roll';
import { isCtaEndCard } from '../domain/cta-end-card';
import { LocalRunnerStore, newPersistedSession } from './store';
import { LOCAL_RUNNER_VERSION, SESSION_BUNDLE_SCHEMA_VERSION, type LocalSessionBundle, type SessionAssetBundle, type StageResult } from './types';
...
export function canonicalBundleHash(bundle: Omit<LocalSessionBundle, 'bundleSha256'> | LocalSessionBundle): string {
...
function validateBundle(bundle: LocalSessionBundle, archiveRoot: string): void {
  ...
  // Previously staged local sessions must remain restartable; new selections use only the active six.
...
export async function stageSessionBundle(bundle: LocalSessionBundle, dependencies: StageDependencies): Promise<StageResult> {
  ...
  try {
    ...
    if (bundle.supportBRollWorkflow) writeFileSync(join(temporary, 'workflow-support-b-roll-t2va.json'), JSON.stringify(bundle.supportBRollWorkflow, null, 2), { flush: true });
